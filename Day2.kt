...
fun partTwo() {
    ...
    File("./src/main/resources/pass.txt").useLines { lines -> lines.forEach {
        val line : List<String> = it.split('-', ' ', ':', ignoreCase = true, limit = 4)
        val positionA : Int = line[0].toInt()
        val positionB : Int = line[1].toInt()
        val currentCharacter : Char = line[2][0]

        if ((line[3][positionA] == currentCharacter && line[3][positionB] != currentCharacter) ||
            (line[3][positionA] != currentCharacter && line[3][positionB] == currentCharacter)) {
            validPasswordCounter++
        }
    }}
